...
class SearchController extends Controller
{
    ...
    {
        $products = Product::where('name', 'like', '%' . addslashes($request->input('q')) . '%')->paginate(config('settings.pagination.items'));
        $term = $request->input('q');

        return view('front.category.search', compact('products', 'term'));
    }
