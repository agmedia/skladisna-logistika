...
@if (isset($blog))
    <div class="row items-push" id="new-images">
        @if (! empty($blog->blocks->groupBy('type')['image']))
            @foreach($blog->blocks->groupBy('type')['image'] as $key => $block)
                @if ($block->type == 'image')
                    <div class="col-lg-3 col-md-4 animated fadeIn mb-30 py-20" id="{{ 'image_id_' . $block->id }}">
                        <div>
                            <div class="slim"
                                 ...
                                 data-meta-type_id="{{ $block->blog_id }}"
...
@push('page_js')
    ...
    <script>
        //
        let blocks = "{{ (isset($blog) && isset($blog->blocks->groupBy('type')['image'])) ? count($blog->blocks->groupBy('type')['image']) : 0 }}";
        let created_id = 0;
        ...
        function removeImage(data, slim) {
            if (data.meta.hasOwnProperty('image_id')) {
                axios.post("{{--{{ route('page.gallery.image.destroy') }}--}}", { data: data.meta.image_id })
                    .then((response) => {
                        successToast.fire({
