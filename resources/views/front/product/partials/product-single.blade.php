...
<div class="product clearfix">
    <div class="product-image loading">
        ...
        <a href="{{ $link }}"><img class="b-lazy" src=data:image/gif;base64,R0lGODlhAQABAAAAACH5BAEKAAEALAAAAAABAAEAAAICTAEAOw== data-src="{{ Image::cache(function ($image) use ($product) { $image->make(asset($product->image))->fit(400, 400)->encode('data-url'); }, env('CACHE_LIFETIME')) }}" data-src-small="{{ Image::cache(function ($image) use ($product) { $image->make(asset($product->image))->fit(400, 400)->encode('data-url'); }, env('CACHE_LIFETIME')) }}" alt="{{ $product->sku }}"></a>
