                ...
                <span class="float-right">
                    <div class="dropdown float-right ml-5">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="products-status" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(48px, 31px, 0px);">
                            ...
                            {{--<button class="dropdown-item" onclick="SelectGroup('tržnica')">
                                <i class="fa fa-fw fa-star text-success mr-5"></i>Tržnica
                            </button>
                            <button class="dropdown-item" onclick="SelectGroup('proizvodnja')">
                                <i class="fa fa-fw fa-star text-success mr-5"></i>Proizvodnja
                            </button>
                            <button class="dropdown-item" onclick="SelectGroup('usluge')">
                                <i class="fa fa-fw fa-star text-success mr-5"></i>Usluge
                            </button>
                            <div class="dropdown-divider"></div>--}}
                        </div>
                    </div>
                    @if (Bouncer::is(auth()->user())->an('admin'))
                        <a href="{{ route('category.create') }}" class="btn btn-secondary ml-30" data-toggle="tooltip" title="New">
                ...
                <table class="js-table-sections table table-hover table-vcenter">
                    ...
                    @foreach($categories as $key => $category)
                        ...
                        <tr>
                            ...
                            <td class="text-center pl-3">{{ $category->subcategory_count }}</td>
                            <td class="text-center text-uppercase font-size-xs">{{ strtoupper($category->group) }}</td>
                            <td class="text-center">{{ $category->sort_order }}</td>
                            @if (Bouncer::is(auth()->user())->an('admin'))
                                <td class="d-none d-sm-table-cell text-right">
                                    <button type="button" class="btn btn-sm btn-circle btn-alt-danger" onclick="event.preventDefault(); shouldDeleteCategory({{ json_encode($category) }});">
                                ...
                                <tr>
                                    ...
                                    @if (Bouncer::is(auth()->user())->an('admin'))
                                        <td class="text-right font-size-sm">
                                            <button type="button" class="btn btn-sm btn-circle btn-alt-danger" onclick="event.preventDefault(); shouldDeleteCategory({{ json_encode($subcategory) }});">
                                                <i class="fa fa-times"></i>
                                            </button>
                                        </td>
                                    @endif
                        ...
                        @else
                            <tr>
                                <td class="text-center"></td>
                                <td class="text-center"></td>
                                <td class="font-w600 text-gray-dark"><span class="text-gray-dark pl-3">Nemate podkategorija!</span></td>
                                <td></td>
                                <td class="text-center text-uppercase font-size-xs"></td>
                                @if (Bouncer::is(auth()->user())->an('admin'))
                                    <td class="d-none d-sm-table-cell text-right">
