                ...
                @else
                    <form action="{{ route('slider.store') }}" method="post" enctype="multipart/form-data">
                        ...
                        <h2 class="content-heading"> <a href="{{ route('sliders') }}" class="mr-10 text-gray font-size-h4"><i class="si si-action-undo"></i></a>
                            @if (isset($slider))
                                Edit Slider Group <small class="text-primary pl-4">{{ $slider->name }}</small>
                            @else
                                Create New Slider Group
                            @endif
                            <button type="submit" class="btn btn-primary btn-sm float-right"><i class="fa fa-save mr-5"></i> Save Slider Group</button>
                        </h2>

                        <div class="block block-rounded block-shadow" id="actions-app">
                            <div class="block-content" id="ag-slider-app">
                                <h5 class="text-black mb-0 mt-20">Slider Group
                                    @if (isset($slider))
                                        <a href="{{ route('slider.edit.sliders', ['id' => $slider->id]) }}" class="btn btn-outline-primary btn-sm float-right"><i class="fa fa-film mr-5"></i> Sliders</a>
                                    @endif
                                </h5>
                                <hr class="mb-30">
                                <div class="row items-push">
                                    <div class="col-lg-3">
                                        <p class="text-muted">General information about the slider group...</p>
                                    </div>
                                    <div class="col-lg-7 offset-lg-1">
                                        ...
                                        <div class="form-group mb-30">
                                            <label class="css-control css-control css-control-success css-switch">
                                                ...
                                                <span class="css-control-indicator"></span> Slider Group Online Status
                                            </label>
                                        </div>
                            ...
                            <div class="block-content block-content-full block-content-sm bg-body-light font-size-sm text-right">
                                <button type="submit" class="btn btn-primary">
                                    <i class="fa fa-save mr-5"></i> Save Slider Group
                                </button>
                            </div>
                        </div>
                    </form>
